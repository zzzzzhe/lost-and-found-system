...
public class UserController {
    ...
    /*---------------------------------账号注册-------------------------------*/
    /*访问"/register"路径时，会返回一个名为"register"的模板页面*/
    @GetMapping("/register")
    public String toRegister(){
        return "register";
    }

    /*当用户以POST方式提交注册表单到"/register"路径时，会执行register()方法*/
    @ResponseBody
    ...
    public String register(@RequestParam("username")String username,
        ...
        user.setUsername(username)
                .setPassword(DigestUtils.md5DigestAsHex(password.getBytes())) //使用DigestUtils类的md5DigestAsHex方法对密码进行加密
                .setEmail(mail)
                .setPhoto("/images/default.jpg")
                .setAdmin("false");
        String code = (String) request.getSession().getAttribute("code");//用于验证用户输入的验证码是否正确
    ...
    public void getCode(HttpServletRequest request,
        ...
        System.out.println("您好，本次验证码为："+code);
        try {
            jmail.send();
